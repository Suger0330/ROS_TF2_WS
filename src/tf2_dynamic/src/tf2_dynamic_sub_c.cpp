#include "ros/ros.h"
#include "tf2_ros/transform_listener.h" //创建订阅对象需要
#include "tf2_ros/buffer.h" //数据缓存需要
...
int main(int argc, char *argv[])
{
    ...
    // 时间戳,设置为(0.0)的原因是因为防止在坐标变换时间戳比对时抛异常
    ...
    while (ros::ok())
    {
        ...
        /*
            ...
            PS2 :运行时存在的问题:抛出一个异常base_ link 不存在
            原因:订阅数据是一个耗时操作,可能再调用transform 转换函数时,坐标系的
                相对关系还没有订阅到，因此出现异常
                解决:
                方案1:在调用转换函数前，执行休眠
                方案2:用 try 进行异常处理
